...
describes what users get. Pick a format with `-o`, or let openbrewery choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
openbrewery <command> -o table   # aligned columns for reading
openbrewery <command> -o jsonl   # one JSON object per line, for piping
openbrewery <command> -o json    # a single JSON array
openbrewery <command> -o csv     # spreadsheet friendly
openbrewery <command> -o tsv     # tab-separated
openbrewery <command> -o url     # just the URL column
openbrewery <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
openbrewery <command> --fields id,title,url
```
...
```bash
openbrewery <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
openbrewery <command>            # a table, because this is a terminal
openbrewery <command> | wc -l    # JSONL, because this is a pipe
```
