// Package cli assembles the openbrewery command tree from the openbrewery
...
// NewApp assembles the kit application from the openbrewery domain. The
...
//
// To add a command, declare it in openbrewery/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := openbrewery.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(openbrewery.Domain{}).Register(app)
