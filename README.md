...
`openbrewery` is a single pure-Go binary. It reads public openbrewery data
over plain HTTPS, shapes it into clean records, and prints output that pipes
into the rest of your tools. No API key, nothing to run alongside it.

The same package is also a [resource-URI driver](#use-it-as-a-resource-uri-driver),
so a host program like [ant](https://github.com/tamnd/ant) can address
openbrewery as `openbrewery://` URIs.
...
Or grab a prebuilt binary from the [releases](https://github.com/tamnd/openbrewery-cli/releases), or run
the container image:
...
to end. Model the real openbrewery records in `openbrewery/` and declare their
operations in `openbrewery/domain.go`; each one becomes a command, an HTTP
...
`openbrewery` registers a `openbrewery` domain the way a program registers a
...
dereferences `openbrewery://` URIs without knowing anything about openbrewery:
